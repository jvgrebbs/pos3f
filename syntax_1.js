...
const state=JSON.parse(localStorage.getItem('lifeOSState')||'{}');

function iso(d){return d.toISOString().slice(0,10)}
function localDate(y,m,d){return new Date(y,m,d)}
const START=localDate(2026,7,17);
const END=localDate(2026,10,8);

function weekFor(d){
  const x=new Date(d.getFullYear(),d.getMonth(),d.getDate());
  const diff=Math.floor((x-START)/86400000);
...
function save(){localStorage.setItem('lifeOSState',JSON.stringify(state))}
...
function getWorkoutLog(){
  return state[strengthKey()] || {};
}

function saveWorkoutLog(log){
  state[strengthKey()] = log;
  save();
...
function render(){
  ...
  document.getElementById('mode').textContent=w?'EXECUTION DAY':(selected<START?'PRE-CAMPAIGN':'CAMPAIGN COMPLETE');
  ...
  document.getElementById('focusText').textContent=w?
    ...
    (selected<START?'Your 12-week campaign begins August 17, 2026.':'The 12-week campaign is complete.');
...
function updateCompletion(tasks,k){
  const done=tasks.filter(t=>state[k+'-'+t[0]]).length, total=tasks.length;
...
function toggle(id,val){state[id]=val;save();render()}
function moveDay(n){
...
window.strengthLogDate=new Date(2026,7,17);
render();
