...
(function(){
  ...
  function syncSelectedDate(){
    try{selected=normalizeDate(selectedDateValue)}catch(e){}
  }
  syncSelectedDate();

  function dateIsToday(){
    const d=selectedDateValue,n=new Date();
  ...
  function dateContext(){
    return selectedDateValue.toLocaleDateString(undefined,{weekday:"long",month:"short",day:"numeric",year:"numeric"});
  }
  function setAppDate(d){
    selectedDateValue=normalizeDate(d);
    localStorage.setItem(DATE_KEY,selectedDateValue.toISOString());
    syncSelectedDate();
    if(typeof window.pos30SyncDateAcrossApp==="function"){
      try{window.pos30SyncDateAcrossApp(selectedDateValue)}catch(e){}
    }
    render();
  }
  ...
  function selectedDate(){return normalizeDate(selectedDateValue)}
