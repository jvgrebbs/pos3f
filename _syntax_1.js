...
function weekFor(d){
  ...
  const diff=Math.floor((x-POS_CAMPAIGN_START)/86400000);
...
function render(){
  ...
  document.getElementById('mode').textContent=w?'EXECUTION DAY':(selected<POS_CAMPAIGN_START?'PRE-CAMPAIGN':'CAMPAIGN COMPLETE');
  ...
  document.getElementById('focusText').textContent=w?
    ...
    (selected<POS_CAMPAIGN_START?'Your 12-week campaign begins August 17, 2026.':'The 12-week campaign is complete.');
